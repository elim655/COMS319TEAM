...
/* const SearchArtist = ({ name, addToCart, removeFromCart, howManyofThis }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('http://localhost:8081/products');
        const data = await response.json();
        const filteredArtists = data.filter(item => item.title.includes(artistName))

        console.log("Filter: " + artistName)
